...
type School = {
  ...
  utilized: number;
  balance: number;
  utilizationPercent: number; // ✅ changed to number (important fix)
};
...
export default function FundManagement() {
  ...
  const [transactionSchool, setTransactionSchool] = useState<School | null>(null);
  ...
  const schools = useMemo<School[]>(() => {
    return Array.from({ length: 300 }, (_, i) => {
      ...
      const utilizationPercent = Number(
        ((utilized / released) * 100).toFixed(1)
      );
  ...
  const filteredSchools = useMemo(() => {
    return roleFiltered.filter((school) => {
  ...
  }, [roleFiltered, searchTerm, mandalFilter, statusFilter]);

  // Reset page on filter change
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, mandalFilter, statusFilter, role]);
  ...
  const generatePDF = (school: School) => {
    ...
    autoTable(doc, {
      ...
      head: [["Field", "Amount"]],
      body: [
        ["Allocated", school.allocated.toLocaleString()],
        ["Released", school.released.toLocaleString()],
        ["Utilized", school.utilized.toLocaleString()],
        ["Balance", school.balance.toLocaleString()],
